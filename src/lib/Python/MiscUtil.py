...
def ProcessOptionOutfileParameters(ParamsOptionName, ParamsOptionValue, InfileName = None, OutfileName = None):
    ...
    """
    ParamsInfo = {'Compute2DCoords' : 'auto', 'Kekulize': False, 'SMILESDelimiter': ' ', 'SMILESIsomeric': True,  'SMILESTitleLine': True, 'SMILESMolName': True, 'SMILESMolProps': False}
    ...
    for Index in range(0, len(ParamsOptionValueWords), 2):
        ...
        if re.match("^(Sanitize|StrictParsing|RemoveHydrogens|Kekulize|SMILESIsomeric)$", ParamName, re.I):
            ...
            if re.match("^Infile$", Mode, re.I):
                ...
                if re.match("^(No|False)$", Value, re.I):
                    ParamValue = False
        elif re.match("^(SMILESMolName|SMILESMolProps)$", ParamName, re.I):
            if re.match("^Outfile$", Mode, re.I):
                if not re.match("^(Yes|No|True|False)$", Value, re.I):
                    PrintError("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is not a valid value. Supported values: Yes No True False" % (Value, Name, ParamsOptionName))
                ParamValue = True
                if re.match("^(No|False)$", Value, re.I):
                    ...
                    PrintError("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is not a valid value during mode %s." % (Value, Name, ParamsOptionName, Mode))
...
def _ProcessInfileAndOutfileAutoParameters(Mode, ParamsInfo, ParamsOptionName, ParamsOptionValue, InfileName, OutfileName):
    ...
    if re.match("^Infile$", Mode, re.I):
        ...
        # SetSMILESMolProps parameter...
        SetSMILESMolProps = False
        if OutfileName is not None:
            SetSMILESMolProps = True if (ParamsInfo["SMILESMolProps"] and CheckFileExt(OutfileName, "smi")) else False
        ParamsInfo["SetSMILESMolProps"] = SetSMILESMolProps
            
def ProcessOptionSeabornPlotParameters(ParamsOptionName, ParamsOptionValue, ParamsDefaultInfo = None):
    ...
    """
    # The default width and height in Matplotlib is 6.4 and  4.8 and maps to aspect ratio of 1.3...
    ParamsInfo = {'Type' : 'auto', 'OutExt': 'svg', 'Width': 'auto', 'Height': 'auto',
    ...
    if ParamsDefaultInfo is not None:
        for ParamName in ParamsDefaultInfo:
            if ParamName not in ParamsInfo:
                PrintError("The default parameter name, %s, specified using \"%s\" to function ProcessOptionSeabornPlotParameters is not a valid name. Supported parameter names: %s" % (ParamName, ParamsDefaultInfo, " ".join(ValidParamNames)))
            ParamsInfo[ParamName] = ParamsDefaultInfo[ParamName]
            
    if re.match("^auto$", ParamsOptionValue, re.I):
        # No specific parameters to process except for parameters with possible auto value...
        _ProcessOptionSeabornPlotParameters(ParamsInfo, ParamsOptionName, ParamsOptionValue, ParamsDefaultInfo = None)
        return ParamsInfo
    ...
    for Index in range(0, len(ParamsOptionValueWords), 2):
        ...
        if re.match("^Style$", ParamName, re.I):
            if not re.match("^(darkgrid|whitegrid|dark|white|ticks)$", Value):
                PrintError("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is not a valid value. Supported values: darkgrid, whitegrid, dark, white or ticks" % (Value, Name, ParamsOptionName))
        elif re.match("^Palette$", ParamName, re.I):
            if not re.match("^(deep|muted|pastel|dark|bright|colorblind)$", Value):
                PrintError("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is not a valid value. Supported values: deep, muted, pastel, dark, bright or colorblind" % (Value, Name, ParamsOptionName))
        elif re.match("^Context$", ParamName, re.I):
            if not re.match("^(notebook|paper|talk|poster)$", Value):
                PrintError("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is not a valid value. Supported values: notebook, paper, talk or poster" % (Value, Name, ParamsOptionName))
        elif re.match("^(Width|Height)$", ParamName, re.I):
            if not re.match("^auto$", ParamValue, re.I):
                Value = float(Value)
                if Value <= 0:
                    PrintError("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is not a valid value. Supported values: > 0" % (Value, Name, ParamsOptionName))
                ParamValue = Value
        elif re.match("^(FontScale)$", ParamName, re.I):
            Value = float(Value)
            if Value <= 0:
                PrintError("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is not a valid value. Supported values: > 0" % (Value, Name, ParamsOptionName))
            ParamValue = Value
...
def _ProcessOptionSeabornPlotParameters(ParamsInfo, ParamsOptionName, ParamsOptionValue, ParamsDefaultInfo = None):
    """Process parameters with possible auto values."""

    if ParamsDefaultInfo is None:
        return

    for ParamName in ParamsInfo:
        ParamValue = ParamsInfo[ParamName]
        if re.match("^auto$", ParamsInfo[ParamName], re.I):
            if ParamName in ParamsDefaultInfo:
                ParamsInfo[ParamName] = ParamsDefaultInfo[ParamName]
